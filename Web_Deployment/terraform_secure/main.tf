...
#These groups will then be hooked up to the load balancer to balance to through a target group and https listener.
resource "aws_lb_target_group" "asg_tg" {
  ...
  tags = {
    Name        = "autoScalingGroupTg"
    Description = "A target group for the https listener to point to the appropriate autoscaling groups."
  }
}
...
#This load balancer will have a listener to switch all http traffic coming through to https
...
#Grab the domain of the site and get the appropriate info for the certificate associated with it.
data "aws_acm_certificate" "alb_ssl" {
  domain = var.alb_domain
}

#Repeat the above with the route53 hosted zone.
data "aws_route53_zone" "certificate_zone" {
  name = var.alb_domain
}

#From there, use the certificate for the https traffic that we'll balance to our instances.
resource "aws_lb_listener" "https_to_tg" {
  load_balancer_arn = aws_lb.alb.arn
  port              = "443"
  protocol          = "HTTPS"
  certificate_arn   = data.aws_acm_certificate.alb_ssl.arn
  default_action {
    type             = "forward"
